...
void CImageInfoDialog::onLoadVolume(const VolumeData &volumeData)
{
	CString tmp;

	// Image name
	tmp = Parser::tstring$CString(volumeData.fileName);
	__ddx_imageName.SetWindowText(tmp);

	// Mem size - width
	tmp.Format(_T("%d"), volumeData.meta.memSize.width);
	__ddx_memSizeW.SetWindowText(tmp);

	// Mem size - height
	tmp.Format(_T("%d"), volumeData.meta.memSize.height);
	__ddx_memSizeH.SetWindowText(tmp);

	// Mem size - depth
	tmp.Format(_T("%d"), volumeData.meta.memSize.depth);
	__ddx_memSizeD.SetWindowText(tmp);

	// Mem size - total
	tmp.Format(_T("%d"), volumeData.meta.memSize.getTotalSize());
	__ddx_memSizeT.SetWindowText(tmp);

	// Spacing - x
	tmp.Format(_T("%f"), volumeData.meta.spacing.width);
	__ddx_spacingX.SetWindowText(tmp);

	// Spacing - y
	tmp.Format(_T("%f"), volumeData.meta.spacing.height);
	__ddx_spacingY.SetWindowText(tmp);

	// Spacing - z
	tmp.Format(_T("%f"), volumeData.meta.spacing.depth);
	__ddx_spacingZ.SetWindowText(tmp);

	// Vol size - width
	tmp.Format(_T("%f"), volumeData.meta.volSize.width);
	__ddx_volSizeW.SetWindowText(tmp);

	// Vol size - height
	tmp.Format(_T("%f"), volumeData.meta.volSize.height);
	__ddx_volSizeH.SetWindowText(tmp);

	// Vol size - depth
	tmp.Format(_T("%f"), volumeData.meta.volSize.depth);
...
BOOL CImageInfoDialog::PreTranslateMessage(MSG* pMsg)
{
