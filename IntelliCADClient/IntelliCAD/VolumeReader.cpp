...
namespace VolumeReader
{
		...
		const int width, const int height, const int depth, const ushort voxelPrecision)
	{
		...
		if (fin)
		{
			...
			retVal.meta.memSize.set(width, height, depth);
	...
	VolumeData readMetaImage(const tstring &path)
	{
		...
		if (header.load(path))
		{
			...
			meta.volSize.set(
				meta.memSize.width * meta.spacing.width,
				meta.memSize.height * meta.spacing.height,
				meta.memSize.depth * meta.spacing.depth
			);
			...
			if (fin)
			{
				const int TOTAL_SIZE = retVal.meta.memSize.getTotalSize();
				retVal.pBuffer = shared_ptr<ushort[]>(new ushort[TOTAL_SIZE]);
				...
				// Padding
				if (retVal.meta.voxelPrecision < USHRT_MAX)
					retVal.meta.voxelPrecision++;
			}
		}
